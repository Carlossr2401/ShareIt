...
export default function MyReservations() {
  ...
  const upcomingReservations = visibleReservations.filter(
    (r) => getReservationDates(r).actualStart > now,
  );
  ...
  const pastReservations = visibleReservations.filter(
    (r) => getReservationDates(r).actualEnd < now,
  );
  ...
  return (
    <Box>
      ...
      >
        <Box>
          <Typography variant="h4" gutterBottom>
            {t("reservations.title") || "My Reservations"}
          </Typography>
          <Typography variant="body1" color="grey.500">
            {t("reservations.subtitle") || "Manage your bookings"}
          </Typography>
        </Box>
        <Button
          variant="outlined"
        ...
        >
          {t("reservations.refresh") || "Refresh"}
        </Button>
      </Box>
      ...
      <Box sx={{ display: "flex", gap: 2, mb: 3, flexWrap: "wrap" }}>
        {[
          {
            label: t("reservations.upcoming") || "Upcoming",
            count: upcomingReservations.length,
            color: "#00E5FF",
          },
          {
            label: t("reservations.active") || "Active",
            count: activeReservations.length,
            color: "#69F0AE",
          },
          {
            label: t("reservations.past") || "Past",
            ...
            <Table>
              ...
              <TableBody>
                ...
                {getDisplayedReservations().map((r) => {
                  ...
                  const rTotal =
                    r.total_amount ||
                    (r.resource?.deposit || 0) + (r.resource?.price || 0);
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell>
                        <Chip
                          label={
                            t(`status.${derivedStatus.toLowerCase()}`) ||
                            derivedStatus
                          }
                          size="small"
                          sx={{
                      ...
                      <TableCell sx={{ fontWeight: 700, color: "#69F0AE" }}>
                        €{rTotal}
                      </TableCell>
                      <TableCell>
                        <Box sx={{ display: "flex", gap: 1 }}>
                          <Tooltip title="Show Check-in QR">
                            <IconButton
                              size="small"
                              onClick={() => setSelectedQrReservation(r)}
                          ...
                          {isCancelable && (
                            <Tooltip
                              title={t("reservations.cancel") || "Cancel"}
                            >
                              <IconButton
                                size="small"
                                onClick={() => handleCancel(r.reservationId)}
          ...
          {selectedQrReservation && (
            <>
              <Box sx={{ bgcolor: "white", p: 2, borderRadius: 2, mb: 3 }}>
                <QRCodeSVG
                  value={`http://localhost:5173/checkin/${selectedQrReservation.reservationId}`}
                  size={200}
                />
              </Box>
              ...
              <Typography variant="body2" color="grey.400" textAlign="center">
                Scan this QR code with the device at the resource to confirm
                your check-in.
              </Typography>
