...
export default function Login() {
  ...
  return (
    <Box sx={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "linear-gradient(135deg, #0A0E1A 0%, #1a1040 50%, #0A0E1A 100%)", position: "relative", overflow: "hidden" }}>
      ...
      <Card component="form" onSubmit={handleLogin} sx={{ maxWidth: 420, width: "100%", mx: 2, p: 2, border: "1px solid rgba(124,77,255,0.2)", boxShadow: "0 8px 32px rgba(124,77,255,0.1)" }}>
        <CardContent sx={{ textAlign: "center" }}>
          ...
          <Typography variant="body2" color="grey.500" sx={{ mb: 2 }}>{t("login.title")}</Typography>
          ...
          <Typography variant="body2" color="grey.500">
            {t("login.noAccount")}{" "}
            <Link component="button" type="button" variant="body2" onClick={() => navigate("/register")} sx={{ color: "secondary.main", cursor: "pointer" }}>{t("login.register")}</Link>
          </Typography>
        </CardContent>
      </Card>
    </Box>
